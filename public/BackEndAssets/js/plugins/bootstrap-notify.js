/*
     ...
     Lines: 236 was changed from top: 5px to top: 50% and we added margin-top:
   -9px. In this way the close button will be aligned vertically Line:219 -
   modified when the icon is set, we add the class "alert-with-icon", so there
   will be enough space for the icon. Lines: 179/222 - class() was changed to
   html() so we can add the Material Design Icons
...
function isDuplicateNotification(notification) {
  ...
  $('[data-notify="container"]').each(function(i, el) {
    ...
    var isSameTitle =
        title === $("<div>" + notification.settings.content.title + "</div>")
                      .html()
                      .trim();
    var isSameMsg =
        message ===
...
function Notify(element, content, options) {
  ...
  // if duplicate messages are not allowed, then only continue if this new
  // message is not a duplicate of one that it already showing
...
$.extend(Notify.prototype, {
      ...
      update : function(command, update) {
        ...
        for (var cmd in commands) {
          ...
          case "icon":
            ...
            break;
          case "progress":
            var newDelay = self.settings.delay -
        ...
        var posX = this.$ele.outerHeight() + parseInt(self.settings.spacing) +
  ...
  placement : function() {
    ...
    if (this.settings.newest_on_top === true) {
      offsetAmt = (parseInt(offsetAmt) + parseInt(this.settings.spacing)) +
    ...
    this.$ele.one(this.animations.start, function() { hasAnimation = true; })
  ...
  bind : function() {
    ...
    this.$ele.mouseover(function() { $(this).data('data-hover', "true"); })
        .mouseout(function() { $(this).data('data-hover', "false"); });
  ...
  reposition : function(posX) {
    ...
    $elements.each(function() {
      ...
      posX = (parseInt(posX) + parseInt(self.settings.spacing)) +
             $(this).outerHeight();
    });
  }
});
