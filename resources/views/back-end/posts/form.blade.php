...
<div class="row my-3">
    ...
    <div class="col-md-12 my-5">
        <div class="form-group bmd-form-group">
            ...
            <textarea  name="{{ $input }}"  cols="30" rows="4" class="form-control @error($input) is-invalid @enderror">{{ isset($row) ? $row->{$input} : '' }}</textarea>
            @error($input)
            <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
             </span>
    ...
    <div class="col-md-12 " >
        <div class="form-group bmd-form-group ">
            <input id="x" class="bg-light" value="{{ isset($row) ? $row->{$input} : '' }}" type="hidden" name="{{$input}}">
